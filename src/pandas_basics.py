...
def load_and_explore_gis_data(file_path):
    ...
    print(f"\n📊 DATASET OVERVIEW")
    print(f"This file has: {df.shape} - {df.shape[0]} rows and {df.shape[1]} columns")
...
def filter_environmental_data(df, min_temp=15, max_temp=30, quality="good"):
    ...
    filtered_df = df[(df['temperature_c'] >= min_temp) & (df['temperature_c'] <= max_temp)]
    ...
    print(f"✅ Filtered data: {len(filtered_df)} rows remain")
    print(f"📉 Remaining: ({(len(filtered_df))/len(df)*100:.1f}%)")
